...
class StrategyToolkit(SophieToolkit):
    ...
    def get_tools(self) -> list[BaseTool]:
        ...
        def list_strategy_presets() -> str:
            ...
            backtest-derived or conventional retail defaults. Only 'iron_condor' is backtested
            (from sophie-option-research's real iron_condor_45dte.yaml config) — do not present any
            other preset's deltas as backtest-justified."""
            lines = []
            rows = []
            ...
            text = "\n".join(lines)
            return ui_envelope(text, "presets_list", presets=rows)
        ...
        def build_strategy(
            ...
            """Resolve a strategy preset against a real chain and price it: legs (in Options
            Viewer OptionLeg shape), net credit/debit, max profit/loss, breakevens, probability of
            profit, and net greeks. `source` is 'live' (Cboe, today) or 'historical' (OptionsDX,
            requires `quote_date`='YYYY-MM-DD'). `delta_overrides` maps a 0-indexed leg position
            (as listed by list_strategy_presets) to a replacement delta target, for what-if
            analysis without hand-computing a whole new preset."""
            try:
                if source == "live":
                    ...
                    if run_ctx.as_of and quote_date > run_ctx.as_of.isoformat():
                        return f"quote_date {quote_date} is after as_of {run_ctx.as_of.isoformat()}; not permitted."
                    chain, spot, iv_lookup = _build_historical_chain(chain_dir, quote_date, target_dte)
                else:
            ...
            handle = store.put(f"strategy_{preset_id}", legs_df)
            g = result["net_greeks"]
            basis = "BACKTESTED (see strategy_backtest_evidence)" if result["backtested"] else "conventional default, not backtested"
            text = (
        ...
        def compare_strategy_variants(
            preset_id: str, leg_index: int, delta_grid: list[float], target_dte: int = 45
        ) -> str:
            """Sweep one leg's delta target across `delta_grid` (e.g. the short strike[s] of an
            iron condor) and compare credit, max loss, credit-to-width, POP, and liquidity for
            each variant. This is what actually answers "which strikes should I pick" — prefer it
            over calling build_strategy repeatedly by hand."""
            if not run_ctx.is_live:
            ...
            handle = store.put(f"variants_{preset_id}", df)
            text = store.preview(handle)
            return ui_envelope(text, "variant_table", preset_id=preset_id, leg_index=leg_index, rows=rows)

        @tool
        def strategy_backtest_evidence(strategy: str) -> str:
            """Look up sophie-option-research backtest runs (option_research_run) for a strategy
            name — params (delta/DTE targets actually used) alongside metrics (Sharpe, CAGR, max
            drawdown, win rate) per config, so a recommendation can cite real evidence instead of
            folklore. `strategy` matches loosely (e.g. 'iron_condor')."""
            try:
            ...
            handle = store.put(f"evidence_{strategy}", df)
            return store.preview(handle)

        return [list_strategy_presets, build_strategy, compare_strategy_variants, strategy_backtest_evidence]
