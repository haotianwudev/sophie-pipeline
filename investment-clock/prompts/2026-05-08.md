...
As of 2026-05-31, the US economy shows the following EWM Z-score cycle signals:
  ...
  Composite Growth Z-score:    +0.158  (above-trend (expansionary))
    [CLI 50% + INDPRO 20% + inv. Jobless Claims 15% + inv. UNRATE 15%]
  Composite Inflation Z-score: +0.117  (above-trend (inflationary))
...
Growth Indicators (latest available):
  ...
  Initial Jobless Claims (ICSA):   200k/week  (inverted; rising = weakening labor; 15% weight)
...
Inflation Indicators (latest available):
  5Y Breakeven Inflation (T5YIE):  2.65%  (market-implied forward inflation; 30% weight)
...
Clock hand position: 36.4 degrees (0 = 12 o'clock, clockwise)
...
Previous Claude evaluation (2026-05-02):
  Phase:     Overheat at 90.0% confidence
  Direction: counterclockwise
  Summary:   • Growth Z +0.147 and Inflation Z +0.169 both positive; clock at 48.98° firmly in Overheat quadrant.
• Counter-clockwise snap: Inflation Z surged from -0.054 in Feb to +0.169 in May via Hormuz blockade.
• CLI 99.85 below 100 threshold; leads growth contraction by 2 quarters per historical pattern.
•
